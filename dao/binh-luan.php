...
function binh_luan_update($ma_bl, $ma_kh, $ma_hh, $noi_dung, $ngay_bl){
    ...
    pdo_execute($sql, $ma_hh, $noi_dung, $ngay_bl, $ma_bl);
}
...
function binh_luan_exits($ma_bl){
    $sql = "SELECT count(*) FROM binh_luan WHERE ma_bl=?";
    return pdo_query_value($sql, $ma_bl)>0;
}
function binh_luan_select_by_hang_hoa($ma_hh){
    $sql = "SELECT b.*, h,ten_hh FROM binh_luan b JOIN hang_hoa h ON h.ma_hh=b,ma_hh WHERE b.mahh WHERE b.ma_hh=? ORDER BY ngay_bl DESC";
